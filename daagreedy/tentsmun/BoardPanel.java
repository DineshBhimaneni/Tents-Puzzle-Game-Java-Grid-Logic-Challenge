...
public class BoardPanel extends JPanel {
    ...
    private final String opponentType; // "Greedy" or "DivideConquer"

    private final int cellSize = 60; // Slightly larger for better visibility
    private final int offset = 80; // Margin for numbers

    private boolean isPlayerTurn = true; // Track whose turn it is
    ...
    public BoardPanel(GameState gameState, GameFrame frame, String opponentType) {
    ...
    private void handleMousePress(MouseEvent e) {
        ...
        if (SwingUtilities.isRightMouseButton(e)) {
            ...
            // Disable player turn and trigger computer move after delay
            isPlayerTurn = false;
            Timer timer = new Timer(1500, evt -> {
                makeAIMove();
                // Re-enable player turn after computer moves
                isPlayerTurn = true;
                repaint();
            });
    ...
    private void handleMouseDrag(MouseEvent e) {
        // Block drag if it's not the player's turn
        if (!isPlayerTurn) {
            return;
        }
    ...
    /**
     * Makes an AI move using the selected opponent type.
     */
    private void makeAIMove() {
        boolean moved = false;
        if (opponentType.equals("Greedy")) {
            moved = GreedyCPU.makeGreedyMove(gameState);
        } else if (opponentType.equals("DivideConquer")) {
            moved = DivideAndConquerCPU.makeMove(gameState);
        }
